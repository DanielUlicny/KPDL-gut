...
// Slovak pluralization: 1→one, 2-4→few, 0/5+→many
export function skPlural(n, one, few, many) {
  if (n === 1) return one;
...
export function Icon({ name, size = 22, stroke = 2, className = '', style = {}, fill = 'none' }) {
...
// ─────────────────────────────────────────────────────────────
export function Card({ children, className = '', style = {}, onClick, padding = true, hover = false }) {
...
// ─────────────────────────────────────────────────────────────
export function Pill({ children, tone = 'soft', size = 'md', icon = null }) {
  const tones = {
...
export function Chip({ children, active = false, onClick, icon = null }) {
  return (
...
export function NumDot({ n, color = 'var(--accent)', size = 36 }) {
  return (
...
export function RegionAvatar({ code, color, size = 40 }) {
  return (
...
// Progress ring
export function ProgressRing({ value, size = 64, stroke = 6, label = null, color = 'var(--accent)' }) {
...
// Image slot
export function ImgSlot({ label = 'photo', children, className = '', style = {}, light = false }) {
  return (
...
// Section header
export function SectionLabel({ children, right = null, className = '' }) {
  return (
